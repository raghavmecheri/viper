...
let rec get_stmt_decs scope stmt =
  ...
  match stmt with
    Block(s_list) -> List.fold_left get_stmt_decs new_scope s_list
  ...
  | For(e1, e2, e3, s) -> 
      ...
      let for_scope = List.fold_left get_expr_decs new_scope expr_list in get_stmt_decs for_scope s
  | DecForIter(ty, name, e, s) -> 
      let iter_scope = add_symbol name ty new_scope in
      let for_scope = get_expr_decs iter_scope e in get_stmt_decs for_scope s
  | While(e, s) -> 
      let while_scope = get_expr_decs new_scope e in get_stmt_decs while_scope s
  | _ -> scope
  in scope
